...
| AI repository operators | Founders, technical operators, research directors, editorial directors, and AI operations leads | [AI Repository Assurance Review](../docs/analysis/ai-repository-assurance-commercial-offer-2026-07-25.md): a sealed, read-only review of declared operating state, controls, and evidence lineage | Founding-pilot hypothesis: `$3,000` fixed; standard-review hypothesis: `$5,000` after proof; neither is approved for sale | Third technical benchmark complete; adjudicate Narrative Systems, run one unrelated benchmark, measure delivery burden, and rehearse a customer-safe packet before any named outreach | Kernel disposition improves from `revise` to `adopt` or commercially bounded `narrow`; accepted-finding precision `>=80%`; trace reconstruction `>=90%`; total delivery `<=8` Anyang hours; approved private-data and commercial-use controls | Medium-high | High if the proof gate passes |
...
2. **AI Repository Assurance Review** has a strong fixed-fee B2B shape and
   ...
   remains pre-commercial while the kernel disposition is `revise`, Narrative
   Systems is unadjudicated, review cost is unmeasured, an unrelated benchmark
   is missing, and private-data and commercial-use controls remain unapproved.
