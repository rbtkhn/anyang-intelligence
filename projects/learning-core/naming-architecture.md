# Learning Core Naming Architecture

This note defines how `Learning Core`, `Learning Core`, and `Student Operating System` should be used inside Anyang Intelligence.

The goal is to keep the lane, the family artifact, and the broader method distinct so they do not blur together in planning, documentation, or parent-facing communication.

All three layers serve one purpose: help a learner build a coherent life of learning. Portability across contexts is a defining principle, and increasing learner ownership is the developmental arc.

## Core Naming Rule

Use the three names like this:

- `Learning Core` = the project lane
- `Learning Core` = the family's operating artifact
- `Student Operating System` = the broader method and doctrine layer

These are related, but they are not interchangeable.

## 1. Learning Core

`Learning Core` names the lane.

Use it when referring to:

- the customer context
- the repo folder
- the offer lane
- the commercial hypothesis
- the service wrapper
- the onboarding and continuity flow as a business lane

Good examples:

- Learning Core is an Anyang Intelligence project lane.
- Learning Core offers a free 30-day collaborative build followed by an optional continuity subscription.
- Learning Core should preserve parent authority and child safety.

Do not use `Learning Core` as the name of the family artifact itself.

## 2. Learning Core

`Learning Core` names the thing the family receives and uses.

It is a portable center of continuity, not a school replacement, single plan, app, portfolio, subscription, or exhaustive record.

Use it when referring to:

- the family's living planning system
- parent-visible plans and rhythms
- observations and follow-up logic
- portfolio and reading-basket interpretation
- recommendation context
- memory, continuity, and handoff surfaces
- selected questions and unfinished learning threads carried across contexts
- learner reflection and increasing ownership of next directions

Good examples:

- The first 30 days should leave the family with a usable `Learning Core`.
- School is somewhere a learner may go; the family's `Learning Core` is something the learner can increasingly take with them.
- The continuity subscription helps keep the `Learning Core` alive.
- The family's `Learning Core` may optionally interface with OB1.

Do not use `Learning Core` as the name of the whole business lane unless the lane is intentionally renamed in a future decision.

## 3. Student Operating System

`Student Operating System` names the broader operating method.

Use it when referring to:

- the doctrine behind the work
- the general model of parent-guided educational support
- readiness, planning, evidence, and review logic
- architecture notes and system design
- reusable method that could later serve more than one project lane

Good examples:

- The `Learning Core` is built using the Student Operating System method.
- The Student Operating System governs how recommendations, observations, and approvals are interpreted.
- The Student Operating System is the method layer, not the household artifact.

Do not use `Student Operating System` as a direct synonym for the family's `Learning Core`.

## Recommended Sentence Pattern

This is the preferred relationship:

- `Learning Core` delivers a 30-day collaborative build of the family's `Learning Core`.
- That `Learning Core` is built using the Student Operating System method.
- The continuity subscription helps the family keep the `Learning Core` alive over time.

## Usage By Surface

Prefer these defaults:

- lane dashboards, portfolio tracking, repo folders, and commercial tables: `Learning Core`
- parent-facing offer copy, onboarding, handoff, continuity, and memory-interface language: `Learning Core`
- doctrine notes, architecture notes, readiness logic, and system-design language: `Student Operating System`

## Why This Matters

If the names collapse together, the lane starts sounding confused:

- the business lane and the household artifact become hard to distinguish
- system doctrine and parent-facing deliverables start sounding like the same thing
- copy becomes clumsy and repetitive

This distinction keeps the lane legible:

- `Learning Core` = where the work lives
- `Learning Core` = what the family uses
- `Student Operating System` = how the system thinks

The 30-day plan, portfolio, reading basket, apps, projects, and reviews are supporting surfaces. None should be described as if it were the whole `Learning Core`.
