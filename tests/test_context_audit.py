...
def test_bounded_grant_and_boundary_in_one_document_are_not_conflict(tmp_path):
    (tmp_path / "policy.md").write_text(
        "This grants authority only within the approved lane. It never grants authority beyond that lane.\n",
        encoding="utf-8",
    )
    report = audit_repository(tmp_path)
