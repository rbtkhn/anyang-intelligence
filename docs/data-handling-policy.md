...
- Pass an explicit external `--db` path, set `ANYANG_DATA_DIR`, or use the
  explicitly reviewed user-local choice configuration. Resolution follows that
  order and must always remain outside the repository.
