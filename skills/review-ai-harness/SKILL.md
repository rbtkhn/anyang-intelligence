...
2. Run:
   ...
   ```text
   .\tools\run.ps1 project harness scan --repo .
   ```

   On macOS or Linux, use `python3 tools/run_repo.py project harness scan --repo .`. The governed runner locates Python, bootstraps declared dependencies outside Git, and loads the current repository source.
...
6. Run:
   ...
   ```text
   .\tools\run.ps1 project harness render --packet <packet> --review <packet>/.harness-review/semantic-review.input.json
   ```
...
```text
.\tools\run.ps1 project harness decide --packet <packet> --approve 1 3 --reject 2
```
