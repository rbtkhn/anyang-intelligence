...
The recursive self-improvement question is:

```text
What did the last cycle teach us that should change how the Executive OS operates next cycle?
```
...
Use this loop inside each `coffee`:

```text
work happens
  -> coffee re-entry
  -> entropy detected
  -> one learning extracted
  -> one improvement candidate named
  -> next work gets smarter
```
...
- Add or revise a template.
- Update a customer operating doc.
- Add a quality gate.
- Change the portfolio priority order.
- Create a new intake or review checklist.

Non-durable thoughts should stay in the `coffee` brief as orientation, not become process.
...
One learning:
- <what the last cycle taught the Executive OS>
