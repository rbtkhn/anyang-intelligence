...
PrivacyScanner = Callable[[str], Collection[str]]


@dataclass(frozen=True)
class ContradictionPolicy:
    """Host-supplied vocabulary and bounds for the pure comparison kernel."""

    ordinary_consequence: str
    consequential_consequence: str
    authority_sensitive_consequence: str
    authority_roles: tuple[str, ...]
    controlling_roles: frozenset[str]
    max_text: int = 500
    max_packet_json: int = 24_000

    @property
    def consequence_levels(self) -> tuple[str, str, str]:
        return (
            self.ordinary_consequence,
            self.consequential_consequence,
            self.authority_sensitive_consequence,
        )

    def __post_init__(self) -> None:
        if any(not value for value in self.consequence_levels):
            raise ValueError("Contradiction policy requires consequence levels")
        if len(set(self.consequence_levels)) != 3:
            raise ValueError("Contradiction policy consequence levels must be distinct")
        if not self.authority_roles:
            raise ValueError("Contradiction policy requires authority roles")
        if not self.controlling_roles:
            raise ValueError("Contradiction policy requires controlling roles")
        if not self.controlling_roles.issubset(self.authority_roles):
            raise ValueError("Controlling roles must be declared authority roles")
        if self.max_text < 1 or self.max_packet_json < 1:
            raise ValueError("Contradiction policy bounds must be positive")
...
def evaluate_contradictions(
    ...
    *,
    ...
    normalized = _validate_packet(packet, policy, privacy_scanner)
    ...
    disposition = _overall_disposition(
        diagnostics, normalized["consequence_level"], policy
    )
...
def _evaluate_assertion(
    ...
    authoritative = [
        ...
        if control["authority_role"] in policy.controlling_roles
    ]
    fresh = [
        control
    ...
    if fresh:
        ...
        controls = same_scope
        needed = "Controlling evidence for the declared operation"
        interaction = "neutral-evidence"
    elif field_controls:
...
def _overall_disposition(
    ...
    if not non_aligned:
        return "continue"
    if consequence_level == policy.authority_sensitive_consequence:
        return "hold"
    if "request-control-conflict" in codes:
        return "clarify"
    if consequence_level == policy.consequential_consequence:
        return "clarify"
...
def _validate_packet(
    ...
    if len(raw) > policy.max_packet_json:
        raise ContradictionPacketError("Contradiction packet is oversized")
    raw_findings = sorted(privacy_scanner(raw))
    if raw_findings:
        raise ContradictionPacketError(
    ...
    request_ref = _text(
        packet.get("request_ref"), "request_ref", maximum=policy.max_text
    )
    scope = _text(packet.get("scope"), "scope", maximum=policy.max_text)
    consequence = _text(
        packet.get("consequence_level"),
        "consequence_level",
        maximum=policy.max_text,
    )
    if consequence not in policy.consequence_levels:
        raise ContradictionPacketError(
    ...
    normalized_assertions = [
        _validate_assertion(value, index, policy)
        for index, value in enumerate(assertions, 1)
    ]
    normalized_controls = [
        _validate_control(value, index, policy)
        for index, value in enumerate(controls, 1)
    ]
    ...
    if len(serialized) > policy.max_packet_json:
        raise ContradictionPacketError("Contradiction packet is oversized")
    findings = sorted(privacy_scanner(serialized))
    if findings:
        raise ContradictionPacketError(
...
def _validate_assertion(
    value: Any, index: int, policy: ContradictionPolicy
) -> dict[str, Any]:
    if not isinstance(value, dict):
    ...
    return {
        "id": _token(
            value.get("id"),
            f"request assertion {index} id",
            maximum=min(120, policy.max_text),
        ),
        "field": _token(
            value.get("field"),
            f"request assertion {index} field",
            maximum=min(120, policy.max_text),
        ),
        "value": _scalar(
            value.get("value"),
            f"request assertion {index} value",
            maximum=policy.max_text,
        ),
        "scope": _text(
            value.get("scope"),
            f"request assertion {index} scope",
            maximum=policy.max_text,
        ),
        "source_ref": _text(
            value.get("source_ref"),
            f"request assertion {index} source_ref",
            maximum=policy.max_text,
        ),
        "provisional": provisional,
    }
...
def _validate_control(
    ...
    if role not in policy.authority_roles:
        raise ContradictionPacketError(
    ...
    return {
        "id": _token(
            value.get("id"),
            f"controlling fact {index} id",
            maximum=min(120, policy.max_text),
        ),
        "field": _token(
            value.get("field"),
            f"controlling fact {index} field",
            maximum=min(120, policy.max_text),
        ),
        "value": _scalar(
            value.get("value"),
            f"controlling fact {index} value",
            maximum=policy.max_text,
        ),
        "scope": _text(
            value.get("scope"),
            f"controlling fact {index} scope",
            maximum=policy.max_text,
        ),
        "authority_role": role,
        "source_ref": _text(
            value.get("source_ref"),
            f"controlling fact {index} source_ref",
            maximum=policy.max_text,
        ),
        "as_of": fact_as_of,
...
def _token(value: Any, label: str, *, maximum: int) -> str:
